...
class XianyuLive:
    ...
    def can_auto_delivery(self, order_id: str) -> bool:
        ...
        # 自动发货为独立授权能力，管理员不受限制，普通租户需使用周/月/季/年卡密开通。
        try:
            owner_id = db_manager.get_cookie_owner_id(self.cookie_id)
            if owner_id is None or not db_manager.has_active_feature_subscription(owner_id, 'auto_delivery'):
                logger.warning(f"【{self.cookie_id}】自动发货授权已到期或未开通，跳过订单 {order_id or '-'}")
                return False
        except Exception as e:
            logger.error(f"【{self.cookie_id}】检查自动发货授权失败，安全起见不执行发货: {self._safe_str(e)}")
            return False
        if not order_id:
    ...
    async def get_ai_reply(self, send_user_name: str, send_user_id: str, send_message: str, item_id: str, chat_id: str):
        ...
        try:
            ...
            # 买家咨询携带商品 ID 时，缺少详情就即时拉取；拉取失败仍保留已有信息继续回复。
            if item_id and (not item_info_raw or not item_info_raw.get('item_detail')):
                fetched_detail = await self.fetch_item_detail_from_api(item_id)
                if fetched_detail:
                    if item_info_raw:
                        await self.save_item_detail_only(item_id, fetched_detail)
                        item_info_raw = db_manager.get_item_info(self.cookie_id, item_id) or item_info_raw
                    else:
                        item_info_raw = {'item_title': f'商品 {item_id}', 'item_price': '未知', 'item_detail': fetched_detail}
                    logger.info(f"【{self.cookie_id}】已为 AI 咨询同步商品详情: {item_id}")
