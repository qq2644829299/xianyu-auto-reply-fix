...
# 短时幂等保护：相同客户端对同一写接口重复提交相同内容时，避免重复创建、扣费或发信。
# 仅保存很短的窗口，正常的后续操作不会被误伤。
request_dedup_cache: Dict[str, float] = {}
request_dedup_lock = asyncio.Lock()
REQUEST_DEDUP_SECONDS = 8
...
@app.middleware("http")
async def prevent_duplicate_submissions(request: Request, call_next):
    """对全站会产生状态变更的 JSON/form 请求做短时幂等保护。"""
    if request.method not in {"POST", "PUT", "PATCH", "DELETE"}:
        return await call_next(request)

    content_type = request.headers.get("content-type", "")
    # 文件上传可能较大，且天然包含随机边界；不在中间件内缓存其请求体。
    ...
    key = f"{request.method}:{request.url.path}:{client_ip}:{fingerprint}"
    now = time.time()
    async with request_dedup_lock:
        expired = [cache_key for cache_key, created_at in request_dedup_cache.items()
                   if now - created_at > REQUEST_DEDUP_SECONDS]
        for cache_key in expired:
            request_dedup_cache.pop(cache_key, None)
        if key in request_dedup_cache:
            return JSONResponse(status_code=409, content={
                "detail": "请求已提交，请勿重复操作",
                "code": "DUPLICATE_SUBMISSION",
            })
        request_dedup_cache[key] = now
    return await call_next(request)
...
# 官网：首页对搜索引擎公开，控制台仍通过 /login.html 和 /admin 访问。
...
async def send_verification_code(request: SendCodeRequest):
    ...
    try:
        ...
        # 发送验证码邮件
        if await db_manager.send_verification_email(request.email, code, request.type):
            return SendCodeResponse(
                success=True,
...
async def register(request: RegisterRequest, http_request: Request):
    ...
    try:
        ...
        # 基础输入校验同时拦截明显的批量/脚本账号；邮箱验证码仍是主校验。
        if (not re.fullmatch(r'[A-Za-z0-9_]{3,32}', username)
                or len(request.password) < 8
                or not re.fullmatch(r'[^@\s]+@[^@\s]+\.[^@\s]+', email)
                or re.search(r'(.)\1{5,}', username)):
            return RegisterResponse(success=False, message="注册信息不符合要求，请检查账号、邮箱和密码")
        if not db_manager.can_register_from_ip(client_ip, days=30):
            logger.warning(f"IP {client_ip} 注册被拦截：30 天内已注册过账号")
            return RegisterResponse(success=False, message="该网络环境 30 天内仅可注册一个账号")
        ...
        if db_manager.create_user(request.username, request.email, request.password):
            ...
            if not user or not db_manager.grant_registration_gift(user['id']):
                logger.error(f"【{request.username}】注册赠送初始化失败")
                return RegisterResponse(success=False, message="注册初始化失败，请稍后重试")
            db_manager.record_registration(client_ip, user['id'], request.username, request.email)
            logger.info(f"【{request.username}】注册成功")
            gift = db_manager.get_registration_gift_settings()
            gift_parts = []
            if gift['ai_credits']:
                gift_parts.append(f"{gift['ai_credits']} 次 AI 回复额度")
            if gift['auto_delivery_days']:
                gift_parts.append(f"{gift['auto_delivery_days']} 天自动发货授权")
            gift_message = f"，已赠送 {' 和 '.join(gift_parts)}" if gift_parts else ''
            return RegisterResponse(
                success=True,
                message=f"注册成功{gift_message}，请登录"
            )
        else:
...
class RegistrationGiftSettingsIn(BaseModel):
    ai_credits: int
    auto_delivery_days: int
...
class AIReplySettings(BaseModel):
    ...
    api_type: str = ""
    manual_pause_minutes: int = 30
    reply_schedule_mode: str = "all_day"  # all_day / custom
    reply_start_time: str = "00:00"
    reply_end_time: str = "23:59"
...
class PlatformAIConfig(BaseModel):
    """平台统一托管的 AI 供应商配置。密钥仅接受管理员写入，永不回传。"""
...
class ActivationCodeBatch(BaseModel):
    product_type: str
    amount: int = 0
    duration_days: int = 0
    count: int = 1
...
class RedeemActivationCode(BaseModel):
    code: str


def _platform_ai_config() -> dict:
    """从平台设置读取 AI 配置，供所有租户的回复任务统一使用。"""
    return {
        'model_name': db_manager.get_system_setting('platform_ai_model_name') or 'deepseek-chat',
        'base_url': db_manager.get_system_setting('platform_ai_base_url') or 'https://api.deepseek.com',
        'api_type': db_manager.get_system_setting('platform_ai_api_type') or 'openai',
        'api_key': db_manager.get_system_setting('platform_ai_api_key') or os.getenv('PLATFORM_AI_API_KEY', '').strip(),
    }


@app.get('/admin/registration-gift-settings')
def get_registration_gift_settings(admin_user: Dict[str, Any] = Depends(require_admin)):
    """管理员读取新用户注册赠送配置。"""
    return db_manager.get_registration_gift_settings()


@app.put('/admin/registration-gift-settings')
def update_registration_gift_settings(settings: RegistrationGiftSettingsIn, admin_user: Dict[str, Any] = Depends(require_admin)):
    """管理员更新后续新注册用户的赠送额度和自动发货授权天数。"""
    if not 0 <= settings.ai_credits <= 100000:
        raise HTTPException(status_code=400, detail='AI 赠送次数需在 0 到 100000 之间')
    if not 0 <= settings.auto_delivery_days <= 3650:
        raise HTTPException(status_code=400, detail='自动发货赠送天数需在 0 到 3650 之间')
    if not db_manager.set_system_setting('registration_gift_ai_credits', str(settings.ai_credits), '新用户注册赠送 AI 回复次数'):
        raise HTTPException(status_code=500, detail='保存 AI 赠送次数失败')
    if not db_manager.set_system_setting('registration_gift_auto_delivery_days', str(settings.auto_delivery_days), '新用户注册赠送自动发货授权天数'):
        raise HTTPException(status_code=500, detail='保存自动发货赠送天数失败')
    return {'message': '注册赠送设置已保存', 'ai_credits': settings.ai_credits, 'auto_delivery_days': settings.auto_delivery_days}


@app.get('/admin/platform-ai-config')
def get_platform_ai_config(admin_user: Dict[str, Any] = Depends(require_admin)):
    """管理员读取平台 AI 配置；只返回密钥是否已配置，避免浏览器暴露密钥。"""
    config = _platform_ai_config()
    return {
...
@app.put('/admin/platform-ai-config')
def update_platform_ai_config(config: PlatformAIConfig, admin_user: Dict[str, Any] = Depends(require_admin)):
    """管理员更新统一 AI 配置；留空 api_key 时保留原密钥。"""
    if not config.model_name.strip() or not config.base_url.strip():
        raise HTTPException(status_code=400, detail='模型名称和 API 地址不能为空')
    db_manager.set_system_setting('platform_ai_model_name', config.model_name.strip(), '平台 AI 模型')
    db_manager.set_system_setting('platform_ai_base_url', config.base_url.strip().rstrip('/'), '平台 AI Base URL')
    db_manager.set_system_setting('platform_ai_api_type', config.api_type.strip() or 'openai', '平台 AI API 类型')
    if config.api_key.strip():
        db_manager.set_system_setting('platform_ai_api_key', config.api_key.strip(), '平台 AI 密钥（仅服务端使用）')
    return {'message': '平台 AI 配置已保存', 'api_key_configured': bool(_platform_ai_config()['api_key'])}


@app.get('/billing/summary')
def get_billing_summary(current_user: Dict[str, Any] = Depends(get_current_user)):
    user_id = current_user['user_id']
    with db_manager.lock:
        cursor = db_manager.conn.cursor()
        cursor.execute('SELECT expires_at FROM user_feature_subscriptions WHERE user_id=? AND feature=?', (user_id, 'auto_delivery'))
        subscription = cursor.fetchone()
        cursor.execute('SELECT created_at, delta, balance_after, note FROM ai_usage_ledger WHERE user_id=? ORDER BY id DESC LIMIT 50', (user_id,))
        ledger = [dict(created_at=r[0], delta=r[1], balance_after=r[2], note=r[3]) for r in cursor.fetchall()]
    return {
        'ai_balance': None if db_manager.is_admin_user(user_id) else db_manager.get_ai_credit_balance(user_id),
        'admin_unlimited': db_manager.is_admin_user(user_id),
        'auto_delivery_expires_at': subscription[0] if subscription else None,
        'usage': ledger,
    }
...
def create_activation_code_batch(payload: ActivationCodeBatch, admin_user: Dict[str, Any] = Depends(require_admin)):
    if payload.count < 1 or payload.count > 500:
        raise HTTPException(status_code=400, detail='单次生成数量需在 1 到 500 张之间')
    if payload.product_type == 'ai_credit' and payload.amount <= 0:
        raise HTTPException(status_code=400, detail='AI 额度卡密必须设置正数额度')
    if payload.product_type == 'auto_delivery' and payload.duration_days not in (7, 30, 90, 365):
        raise HTTPException(status_code=400, detail='自动发货卡密仅支持周(7)、月(30)、季(90)、年(365)')
    codes = db_manager.create_activation_codes(payload.product_type, payload.amount, payload.duration_days, payload.count)
    return {'message': f'已生成 {len(codes)} 个卡密', 'codes': codes}


@app.get('/admin/activation-codes')
def list_activation_codes(admin_user: Dict[str, Any] = Depends(require_admin)):
    with db_manager.lock:
        cursor = db_manager.conn.cursor()
        cursor.execute('''SELECT id,code,product_type,amount,duration_days,status,used_by,used_at,created_at
                          FROM activation_codes ORDER BY id DESC LIMIT 500''')
        columns = ['id','code','product_type','amount','duration_days','status','used_by','used_at','created_at']
...
def get_ai_reply_settings(cookie_id: str, current_user: Dict[str, Any] = Depends(get_current_user)):
    ...
    try:
        ...
        is_admin = db_manager.is_admin_user(user_id)
        # 租户只管理回复策略。管理员可在同一表单维护平台模型与 API 密钥（密钥只写入，不回显）。
        platform = _platform_ai_config()
        settings.update({
            ...
            'platform_ai_ready': bool(platform['api_key']),
            'can_manage_platform_ai': is_admin,
            'manual_pause_minutes': db_manager.get_cookie_pause_duration(cookie_id),
        })
        return settings
    except HTTPException:
        raise
...
def update_ai_reply_settings(cookie_id: str, settings: AIReplySettings, current_user: Dict[str, Any] = Depends(get_current_user)):
    ...
    try:
        ...
        is_admin = db_manager.is_admin_user(user_id)
        # 管理员在这里更新平台统一 API；租户提交的供应商字段一律丢弃。
        if is_admin:
            platform_updates = {
                'platform_ai_model_name': settings.model_name.strip(),
                'platform_ai_base_url': settings.base_url.strip(),
                'platform_ai_api_type': settings.api_type.strip() or 'openai',
            }
            for key, value in platform_updates.items():
                db_manager.set_system_setting(key, value)
            if settings.api_key.strip():
                db_manager.set_system_setting('platform_ai_api_key', settings.api_key.strip())

        settings_dict = settings.dict()
        settings_dict['api_key'] = ''
        settings_dict['model_name'] = 'platform-managed'
        settings_dict['base_url'] = ''
        settings_dict['api_type'] = 'platform'
        pause_minutes = max(0, min(1440, int(settings.manual_pause_minutes)))
        db_manager.update_cookie_pause_duration(cookie_id, pause_minutes)
    ...
    try:
        ...
        for cid, settings in all_settings.items():
            ...
            settings.update({
                ...
                'platform_ai_ready': bool(platform['api_key']),
                'can_manage_platform_ai': is_admin,
                'manual_pause_minutes': db_manager.get_cookie_pause_duration(cid),
            })
